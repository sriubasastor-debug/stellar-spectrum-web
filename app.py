...
def plot_hr_multi(temps, lums, categories):
    ...
    ax.fill_between(tgrid, 0.1*L_ms, 10*L_ms, color="#4ea3ff", alpha=0.12, label="主序带")
    ax.fill_between(tgrid, 10*L_ms, 1e8, color="#ffdd77", alpha=0.12, label="巨星区")

    mask = tgrid > 5000
    ax.fill_between(tgrid[mask], 1e-8, 0.05*L_ms[mask], color="#cda8ff", alpha=0.12, label="白矮星区")
...
def csv_page():
    if request.method == "POST":
        ...
        hr_image = plot_hr_multi(temps, lums, cats)
